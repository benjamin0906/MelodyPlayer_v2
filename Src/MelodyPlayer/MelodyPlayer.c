...
static const float32 NoteLookup[] = {0,      16.35,  17.32,  18.35,  19.45,  20.60,  21.83,  23.12,  24.50,  25.96,
                                    ...
                                    2793.83,2959.96,3135.96,3322.44,3520.00,3729.31,3951.07,4186.01,4434.92,4698.63,
                                    4978.03,5274.04,5587.65,5919.91,6271.93,6644.88,7040.00,7458.62,7902.13
};
...
void MelodyPlayer_Task(void)
{
	...
	case NoteChange:
		if(NoteCounter < CurrentMelody.Length)
		{
			...
			SineGen_Apply(NoteLookup[CurrentMelody.Notes[NoteCounter].MusicNote]);
	...
	case Wait:
		...
		break;
	case SilentWait:
	    if(IsPassed(Time, Timeout)) State = NoteChange;
	    break;
	}
}
